...
class Transformer(object):
    ...
    def __init__(self, graph: pypendency.models.graph.Graph):

        self.slug_map = {node.slug: node for node in graph.nodes}

    def create_relations_from_raw_relations(self, raw_relations: List[RawRelation]) -> List[generics.Relation]:
        res = []
    ...
    def create_relations_from_raw_relation(self, raw_relation: RawRelation ) -> List[generics.Relation]:
        ...
        relations = self.__create_relation(from_node, to_node, raw_relation.label, direction)
        return relations
    ...
    def __create_relation(self,from_nodes, to_nodes, label, direction) -> List[generics.Relation]:
        relations = []
        for n1 in self._extract_tokens(from_nodes):
            for n2 in self._extract_tokens(to_nodes):

                relation = generics.Relation(self.slug_map[n1], self.slug_map[n2], label, direction)
